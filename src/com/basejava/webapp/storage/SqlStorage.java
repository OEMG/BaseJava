...
public class SqlStorage implements Storage {
        ...
        return sqlHelper.transactionalExecute(conn -> {
            ...
            try (PreparedStatement ps = conn.prepareStatement("SELECT * FROM resume ORDER BY full_name, uuid")) {
                ...
                while (rs.next()) {
                    ...
                    resumes.computeIfAbsent(uuid, k -> new Resume(uuid, full_name));
                }
            }
    ...
    private void deleteFromTable(String tableName, Resume resume) {
        sqlHelper.execute("DELETE FROM " + tableName + " WHERE resume_uuid = ?", ps -> {
            ps.setString(1, resume.getUuid());
            ps.execute();
            return null;
        });
